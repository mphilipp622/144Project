...
BlockingQueue::BlockingQueue(int consumerInterval, int producerInterval) : BlockingQueue()
{
	...
	output << "capacity,timestamp,countFull,countEmpty\n";
...
void BlockingQueue::WriteDataToCSV()
{
	...
	auto elapsed = end - startTime;

	output << to_string(GetPercentageFull() * 100) + "," + to_string(elapsed.count()) + "," + to_string(countFull) + "," + to_string(countEmpty) + "\n";
}